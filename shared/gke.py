"""
GKE job spawner — shared between launcher and execution service.

Spawns a new execution pod (Kubernetes batch/v1 Job) for a given JobRecord.
Used by:
  - services/launcher: to start the first job after upload validation
  - services/execution: to auto-start the next PENDING job after completion
"""
...
def spawn_execution_job(job: JobRecord, prompt_count: int, prompts_path: str) -> None:
    ...
    Idempotent: if a Job with the same name already exists (409 Conflict),
    the error is silently swallowed — the pod is already running.
    """
    ...
    try:
        ...
        if exc.status == 409:
            return  # already running — Eventarc/queue retry
        raise
...
def _build_job_manifest(
    ...
    env_vars = [
        ...
        # Passed down so execution pod can spawn the next queued job (Phase 8)
        k8s.V1EnvVar(name="GKE_CLUSTER_ENDPOINT",  value=os.environ.get("GKE_CLUSTER_ENDPOINT", "")),
        k8s.V1EnvVar(name="EXECUTION_IMAGE",       value=os.environ.get("EXECUTION_IMAGE", "")),
    ]
    if job.api_key_ref:
